...
# Get labels
TRAIN_NUM = 9800
LABELS = np.empty([TRAIN_NUM, 1], dtype=int)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        if batch_idx % 100 == 0:
            log_string('[0:d]  mean loss: {1:f}     accuracy: {2:f}'.format(batch_idx, loss_sum / float(batch_idx+1), total_correct / float(total_seen)))

    log_string('[0:d]  mean loss: {1:f}     accuracy: {2:f}'.format(num_batches, loss_sum / float(batch_idx+1), total_correct / float(total_seen)))
