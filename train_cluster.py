...
def train_one_epoch(sess, ops, train_writer):
    ...
    current_data_1 = np.empty([3*len(LABELS), NUM_POINT, 3], dtype=float)
    current_data_2 = np.empty([3*len(LABELS), NUM_POINT, 3], dtype=float)
    current_label  =  np.empty([3*len(LABELS),1], dtype=int)

    fn = 0
    count = 0
    while fn < len(LABELS) - 1:
        f1, f2 = fn, fn+1
        fn = fn + 2
